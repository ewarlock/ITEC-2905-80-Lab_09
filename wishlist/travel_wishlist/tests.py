...
class TestHomePage(TestCase):
    ...
    def test_home_page_shows_empty_list_message_for_empty_db(self):
        ...
        # check correct template shown, and check correct response received
...
class TestAddNewPlace(TestCase):
    ...
    def test_add_new_unvisited_place(self):
        ...
        response = self.client.post(add_place_url, new_place_data, follow=True) # if another request made from first request, follow redirect

        self.assertTemplateUsed(response, 'travel_wishlist/wishlist.html')

        response_places = response.context['places'] # contect is dictionary of what view is sending to template
        self.assertEqual(1, len(response_places)) # check only 1 place sent
...
class TestVisitPlace(TestCase):
    ...
    def test_visit_place(self):
        visit_place_url = reverse('place_was_visited', args=(2, )) # arge generates the right url with the example in args
        # so we are looking for pk 2
        # has to be tuple
